...
const TreeView: FC<treeViewProps> = (props) => {
  ...
  const checkBoxRender = useCallback(
    (treeData: treeData) => {
      ...
      if (treeData.disabled) {
        return <div className="disblaed-checkBox" />;
      }
      if (!treeData?.children?.length) {
        ...
        // 未选中
        return <div className="checkBox-noActived" onClick={() => checkTreeNode(treeData)} />;
      }
      if (treeData.children && treeData.children.length) {
        // 有子节点
        const treeList: Array<number> = []; // 0 -> 子节点未选中，1->  子节点选中
          ...
          for (let i = 0; i < (treeNode.children as Array<treeData>).length; i++) {
            ...
            if (i === (treeNode.children as Array<treeData>).length - 1) {
              if (treeList.every((c) => c === 0)) {
                // 全都没选中
                return (
                  <div className="checkBox-noActived" onClick={() => checkTreeNode(treeData)} />
                );
              }
              if (treeList.every((c) => c === 1)) {
                // 全都选中
                return (
                  <div className="checkBox-actived" onClick={() => checkTreeNode(treeData)}>
                    <CheckOutlined />
  ...
  const drop = (e: any, treeNode: treeData) => {
    ...
    if (!avaDrop) return;
    e.nativeEvent.preventDefault();
    let dragTreeNode = e.nativeEvent.dataTransfer.getData('dargTree'); // 被拖拽的树节点
    if (!dragTreeNode) {
      return;
    }
    const oldStateTree = [...stateTreeData];
